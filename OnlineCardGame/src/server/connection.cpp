...
int create_connection()
{
    ...
    if (sock == -1)
    {
        printAndWriteLog(SERVER_TAG, "Blad socketu");
        exit(1);
    }
    ...
    {
        printAndWriteLog(SERVER_TAG, "Blad bindingu socketu");
        exit(1);
    }
    ...
    {
        printAndWriteLog(SERVER_TAG, "Blad getsockname");
        exit(1);
    }
    printAndWriteLog(SERVER_TAG, "Port nasluchujacy:");
    std::string s = std::to_string(ntohs(server.sin6_port));
...
void listen_connections(int sock, char haslo[],int iluGraczy)
{
    ...
    do 
    {
    	...
    	{
    		printAndWriteLog(SERVER_TAG, "Blad alokacji pamieci");
    		exit(1);
    	}
        new_client->socket = accept(sock,(struct sockaddr *) 0,(socklen_t *) 0);
        if (new_client->socket == -1 )
        {
            printAndWriteLog(SERVER_TAG, "Blad accept");
        }
        else
        {   
                ...
                if(istniejeGracz==1)
		{
		     write(new_client->socket,"1",2);
                     printAndWriteLog(new_client->nick, "Podany gracz istnieje");  
                     if(( recv( new_client->socket,new_client->nick, 50, 0 ) ) <= 0 )
            	     { 
        		printAndWriteLog(SERVER_TAG, "Blad recv" );
        		exit( - 1 );
		...
		if(( recv( new_client->socket, haslo2, 201, 0 ) ) <= 0 )
                ...
                printAndWriteLog(new_client->nick, "Podano haslo");
		...
		write(new_client->socket, "Witaj, chcesz byc graczem (1) czy moze widzem(2). Wybierz  odpowiedni numer:",81 );
            	if(( recv( new_client->socket, trybGracza, 2, 0 ) ) <= 0 )
        	{
                ...
                printAndWriteLog(new_client->nick, "Tryb gracza:");
                printAndWriteLog(new_client->nick, trybGracza);
