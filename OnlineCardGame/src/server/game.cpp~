...
int game(std::vector<struct client_t*> clientList, int iloscGraczy,bool &czyKoniec,std::vector<struct client_t*> chatList)
{
	...
	for(int i=0;i<clientList.size();++i)
	{
		...
		printAndWriteLogString(clientList[i]->nick, "Karty gracza:\n" + s);
	}
	sleep(1);
	...
	printAndWriteLog(SERVER_TAG, "KONIEC GRY\n");
	czyKoniec = true;
}
...
int ruchGracza(int numerGracza , std::vector<struct client_t*> &clientList,std::vector<karta> &stosKart,std::vector<struct client_t*> chatList)
{
	...
	if(( recv( clientList[numerGracza]->socket, b, sizeof( b ), 0 ) ) <= 0 )
  	{
   		printAndWriteLog(SERVER_TAG, "Blad recv");
    		exit( - 1 );
  	}
	printAndWriteLog(clientList[numerGracza]->nick, b);
...
void wyslijWszystkimStanStosu(std::vector<struct client_t*> &clientList,std::vector<karta> &stosKart,std::vector<struct client_t*> chatList)
{
	...
	printAndWriteLogString(SERVER_TAG, "Stan stosu: " + s);
}
...
//std::cout<<"na stos jest polozona karta "+clientList[numerGracza]->kartyGracza[numerKartyGracza].getNumer() + " " + clientList[numerGracza]->kartyGracza[numerKartyGracza].getTyp()<<std::endl;
	...
	{
		printAndWriteLog(clientList[numerGracza]->nick, "Gracz skonczyl gre");
		//clientList.erase(clientList.begin()+numerGracza);
	}
	char numer[24];
	sprintf(numer,"gracz %d polozyl karte : ",numerGracza);
	
	printAndWriteLogString(clientList[numerGracza]->nick, "Gracz polozyl karte: " + stosKart[stosKart.size()-1].getNumer()+" "+stosKart[stosKart.size()-1].getTyp());
...
int pobierzKartyZeStosu(int numerGracza , std::vector<struct client_t*> &clientList,std::vector<karta> &stosKart,std::vector<struct client_t*> chatList)
{
	...
	std::string karty = clientList[numerGracza]->kartyGracza[clientList[numerGracza]->kartyGracza.size()-1].getNumer()+" "+clientList[numerGracza]->kartyGracza[clientList[numerGracza]->kartyGracza.size()-1].getTyp()+" "+
...
clientList[numerGracza]->kartyGracza[clientList[numerGracza]->kartyGracza.size()-3].getNumer()+" "+clientList[numerGracza]->kartyGracza[clientList[numerGracza]->kartyGracza.size()-3].getTyp();
	...
	printAndWriteLogString(clientList[numerGracza]->nick, "Gracz pobral karty: " + karty);

	for(int i = 0 ; i < clientList.size() ; i++)
	{
		if(i!= numerGracza)
			wyslij(clientList[i],std::string(numer)+karty+"\n");
	}
	for(int i = 0 ; i < chatList.size() ; i++)
	{
		wyslij(chatList[i],std::string(numer)+karty+"\n");
	}
...
std::vector<struct client_t*> rozlosujKarty(std::vector<struct client_t*> clientList,std::vector<karta> wszystkieKarty, int &numerGraczaKtoryRozpoczyna)
{
	...
	for(int i = 0 ; 0 < wszystkieKarty.size(); i++)
	{
		...
		if(!czyWiemyKtoZaczyna)
		{
			...
			{
				printAndWriteLogString(SERVER_TAG, "Rozpoczyna gracz " + std::string(clientList[i]->nick));
